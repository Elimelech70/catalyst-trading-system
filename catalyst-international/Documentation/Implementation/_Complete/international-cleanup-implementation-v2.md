...
### Step 5.0: IMPORTANT - Audit Existing Code First

**Before making changes, Claude Code must audit the current state:**

```bash
# Check what pools currently exist
grep -n "create_pool\|db_pool\|research_pool\|trading_pool" position_monitor_service.py

# Check what the __init__ or start method looks like
grep -A 20 "def __init__\|async def start" position_monitor_service.py
```

**Key Questions:**
1. Does `self.db_pool` already exist for trading DB?
2. Does `self.research_pool` exist for consciousness?
3. How is the pool passed in vs created internally?

**Adaptation Rules:**
- If `self.db_pool` already exists → USE IT for db_logger, don't create new
- If `self.research_pool` exists → REMOVE it
- If pools are passed in via constructor → Update the caller, not the service

### Step 5.1: Remove Research Database Pool

```python
# REMOVE: Research pool initialization (if present)
...
# REMOVE: Research pool close (if present)
if self.research_pool:
    await self.research_pool.close()

# REMOVE: Any self.research_pool references
```
...
**IMPORTANT:** Use the EXISTING `self.db_pool` - do NOT create a new one.

```python
# ADD: Import at top
from db_logger import DatabaseLogHandler, setup_db_logging

# ADD: In initialization - USE EXISTING POOL
# Look for where self.db_pool is already set up, then add after it:

async def start(self):
    # ... existing db_pool setup (KEEP AS IS) ...
    
    # ADD AFTER existing pool setup:
    # Setup database logging using existing pool
    self.db_log_handler = setup_db_logging(
        self.db_pool,  # Use existing pool, don't create new!
        'position_monitor',
        level=logging.INFO,
        fallback_log_dir='/root/catalyst-international/logs'
    )
    logger.info("Position monitor started with database logging")
```

**If pool is passed in via constructor:**

```python
class PositionMonitorService:
    def __init__(self, db_pool, ...):  # Pool passed in
        self.db_pool = db_pool
        # ... other init ...
        
    async def start(self):
        # Setup logging with the passed-in pool
        self.db_log_handler = setup_db_logging(
            self.db_pool,
            'position_monitor',
            level=logging.INFO
