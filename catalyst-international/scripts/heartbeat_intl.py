#!/usr/bin/env python3
"""
Catalyst Trading System - INTL Claude Heartbeat with Task Execution
Name of file: heartbeat_intl.py
Version: 2.0.0
Last Updated: 2025-12-31
Purpose: Autonomous heartbeat for intl_claude with task execution

Runs on INTL droplet, handles HKEX trading tasks
"""
...
from task_executor_intl import TaskExecutor, parse_task_message, WHITELIST
...
CHANGELOG_PATH = "/root/Catalyst-Trading-System-International/catalyst-international/CHANGELOG-AUTO.md"
    ...
    try:
        if not os.path.exists(CHANGELOG_PATH):
            ...
            with open(CHANGELOG_PATH, 'w') as f:
                f.write("# Catalyst INTL Auto-Generated Changelog\n\n")
                f.write("*Automatically updated by intl_claude when files are modified.*\n\n")
                f.write("---\n\n")
...
async def send_task_report(pool, to_agent: str, task_name: str, msg_subject: str, result: dict):
    ...
    report_lines = [
        ...
        f"**Executed by:** intl_claude (HKEX)",
        f"",
    ]
...
async def heartbeat():
    ...
    try:
        ...
        prompt = f"""You are intl_claude, the HKEX trading assistant on the INTL droplet.
Current time: {datetime.now().isoformat()}
Messages processed this cycle: {len(messages)}
Task results: {len(task_results)}

If there were tasks, summarize what was done. Otherwise note you're operational and monitoring HKEX.
