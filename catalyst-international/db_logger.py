...
REVISION HISTORY:
v1.2.0 (2026-02-01) - Fixed event loop issue
  - Changed from asyncpg to psycopg2 (synchronous) in worker thread
  - Removed async/event loop dependencies
  - Worker thread now has its own DB connection
...
Features:
- Non-blocking writes via background thread with psycopg2
...
class DatabaseLogHandler(logging.Handler):
    ...
    def __init__(
        ...
        # Worker thread database connection (created in worker)
        self._conn = None
    ...
    def _worker_loop(self):
        ...
        Uses synchronous psycopg2 connection (no event loop issues).
        """
    ...
    def _get_connection(self):
        """Get or create database connection for worker thread."""
    ...
    def _flush_batch_sync(self, batch: List[Dict]):
        """
        Flush batch synchronously using psycopg2.
    ...
    def _write_batch(self, batch: List[Dict]):
        """Synchronous batch insert to database using psycopg2."""
        if not batch:
            return
