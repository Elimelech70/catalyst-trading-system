...
| 1.1.0 | 2026-05-18 | Craig + Claude | Schema home moved to the dedicated `catalyst_research` database (matching architecture v1.2.0). `cr_` table prefix dropped — namespace isolation comes from the DB. |
| 1.2.0 | 2026-05-18 | Craig + Claude | Schema home moved back to `catalyst_intl` (matching architecture v1.3.0). Shared `securities`, `exchanges`, and one Moomoo ingestion serving both research and intl trading. The `cr_` prefix returns on research-only fact tables; shared dimensional tables (`countries`, `sectors`, `themes`, `commodities`) carry no prefix. Phase 0.3 rewritten to extend `catalyst_intl` rather than drop the legacy `catalyst_research` DB. Phase 1 DDL rewritten with `ALTER TABLE` clauses for `securities`/`exchanges` and full `cr_*` table definitions. Phase 1 also introduces four PostgreSQL roles (`catalyst_research_admin`, `catalyst_research_ingestion`, `catalyst_research_archetype`, plus confirming `catalyst_trading_writer`). Phase 4.3 env vars consolidated to use `INTL_DATABASE_URL`. Phase 4.5 cleanup adds dropping the unused `catalyst_research` cluster DB and `catalyst_dev`. |
...
- **Phase 0** — preparation: credential rotation, DB snapshot, role planning
...
DigitalOcean managed PostgreSQL snapshot of `catalyst_intl` taken before any DDL runs. This is the primary safety net — Phase 1 extends the live trading DB with research tables and `ALTER TABLE` statements on `securities` and `exchanges`. The snapshot is the rollback path if anything goes wrong.
...
### 0.3 Confirm the intl trading state before extending its database

Phase 1 will add columns to live intl trading tables (`securities`, `exchanges`) and add new tables alongside them. Before doing this, confirm intl trading is in a known state:

```bash
# All ALTER TABLE statements in Phase 1 are additive (ADD COLUMN, no DROP / no type change).
# This means intl trading code will continue to run unmodified after Phase 1.
# But confirm no schema migrations are in flight on the intl side:
cd /root/catalyst-international && git status
# Expected: clean working tree on main

# Confirm Moomoo OpenD is reachable from the intl droplet (this will be the data source
# for cr_security_prices and cr_news_events ingestion in Phase 2):
...
> ⚠️ Phase 1 modifies the live trading database (`catalyst_intl`). All Phase 1 statements are additive — no column drops, no type changes, no row deletions. Intl trading code is unaffected. But the snapshot from 0.1 is the rollback path if a constraint or grant statement misfires.

**Exit criteria:** snapshot of `catalyst_intl` exists from today, credentials rotated, intl trading repo clean, Moomoo OpenD reachable, `INTL_DATABASE_URL` set in the droplet `.env`.
...
It is applied to `catalyst_intl` as a single transaction. If any statement fails, the entire migration rolls back and the snapshot from Phase 0.1 stays as the second-line fallback.
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'catalyst_research_admin') THEN
        CREATE ROLE catalyst_research_admin LOGIN PASSWORD :'admin_pwd';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'catalyst_research_ingestion') THEN
        CREATE ROLE catalyst_research_ingestion LOGIN PASSWORD :'ingestion_pwd';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'catalyst_research_archetype') THEN
        CREATE ROLE catalyst_research_archetype LOGIN PASSWORD :'archetype_pwd';
    END IF;
END $$;
...
-- Security prices: FK to the SHARED intl securities table.
-- One Moomoo ingestion writes here; both research and intl trading code read.
...
-- ingestion role: INSERT only on research fact tables; SELECT on dimensional tables;
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO catalyst_research_ingestion;

-- archetype role: SELECT on everything (including intl trading tables, so archetypes
-- can learn from real trade outcomes); INSERT on its own analysis tables only.
...
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO catalyst_research_archetype;

-- Future-proofing: ensure new tables created by the admin role get the right defaults.
...
- The 20–30 HKEX securities — via `INSERT ... ON CONFLICT DO NOTHING` against the existing `securities` table, then `UPDATE` to set `listing_country` and `primary_sector_id` for them. Any of these already present from intl trading remain in place with their existing `security_id`.
- The three v1 learning plans as `cr_learning_plans` rows (full text from architecture Section 7).
- `country_code` populated on the existing HKEX row in `exchanges`.

The seed script runs as `catalyst_research_admin` (it touches both `securities` and `cr_*` tables) and uses `INSERT ... ON CONFLICT DO NOTHING` and idempotent `UPDATE` so re-running is safe.
...
- `002_seed_v1.sql` applied; the four countries, four commodities, three learning plans, and 20–30 securities exist.
- A read-only smoke query as `catalyst_research_ingestion` confirms it can read `securities` but not the trading tables:
  ```sql
...
| 2/3 | `ingest_security_prices_daily` | Moomoo OpenD (intl droplet) | Daily, post-HKEX-close | `30 8 * * 1-5` UTC (HKEX 16:00 HKT close) | `cr_security_prices` |
...
The order shakes down the adapter pattern on the simplest layer first, then proceeds country-by-country per architecture Section 4:
...
The wrapper script `catalyst-research/archetypes/run.py` orchestrates this — it builds the context bundle, invokes Claude Code, and writes the resulting analysis row into `cr_archetype_analyses` via the `catalyst_research_archetype` role.

> 🔍 The exact `claude` CLI flag set above is illustrative and must be smoke-tested against the Claude Code version installed on the intl droplet before being committed to cron. Flag names and `--max-turns` defaults change between Claude Code releases. Verifying this is a Phase 3 entry criterion.
...
├── db.py               ← read-only DB adapter (connects as catalyst_research_archetype)
...
Craig reviews proposals via `scripts/show_model_proposals.py` (see Section 4.4).
...
- `claude` CLI flags smoke-tested and confirmed against the installed Claude Code version on the intl droplet.
...
│   │   ├── show_cron_health.py           ← heartbeat: last run + status per job
...
Installed as `/etc/cron.d/catalyst-research` on the **intl droplet** (because Moomoo OpenD is local there, and co-locating all jobs keeps operational concerns in one place):
...
- `scripts/show_cron_health.py` — heartbeat. For each cron'd ingestion job, shows the most recent successful insert date and row count. The doc commits to no alerting (Appendix B), so this is Craig's discovery mechanism for silently-failing jobs. Run weekly alongside the weekly report.
...
5. Reclaim US droplet disk: prune Docker images, orphaned volumes, build cache (architecture estimates ~21 GB recoverable).
...
- `show_cron_health.py` confirms no silent-failure jobs across the four-week window.
...
| 3 | Moomoo OpenD | local daemon on intl droplet | Yes | Reused from intl; security prices and bars; one ingestion serves both subsystems |
...
| Week | Phase | Output |
|---|---|---|
| 1 | Phase 0 + Phase 1 | Snapshots, credentials rotated, schema applied, roles created, seed data in place |
...
| 6 | Phase 3 + Phase 4 | First archetype runs; cron file installed |
