...
class TradeWatchdog:
    ...
    async def check_stuck_orders(self) -> List[Dict[str, Any]]:
        ...
        stuck = await self.db.fetch(f"""
            ...
            WHERE o.status IN ('submitted', 'pending_new', 'accepted', 'new')
    ...
    async def check_order_sync(self) -> List[Dict[str, Any]]:
        ...
        # Get recent non-terminal orders from DB
        ...
        for row in db_orders:
            try:
                ...
                if alpaca_status != row['db_status']:
                    fix_parts = [f"UPDATE orders SET status = '{alpaca_status}'"]
                    ...
                    fix_sql = f"UPDATE orders SET status = '{alpaca_status}', {fix_sql} WHERE order_id = '{row['order_id']}'"
